...
function Visualizer({ level, colorClass = "bg-accent", pulse = false, count = 52, maxHeight = 24 }: { level: number, colorClass?: string, pulse?: boolean, count?: number, maxHeight?: number }) {
  const scaledLevel = Math.min(Math.pow(level, 0.4) * 2.5, 1.0);
  const halfCount = count / 2;
  const maxBar = maxHeight || 24;
  
  return (
    <div className={`flex items-center justify-center gap-0.5 h-full w-full ${pulse ? 'animate-pulse opacity-80' : ''}`}>
      {Array.from({ length: count }).map((_, i) => {
        const rand = pulse ? 0.8 : Math.abs(Math.sin(i * 0.73)) * 0.6 + 0.4;
        const centerDist = Math.abs(halfCount - i) / halfCount;
        const envelope = 1.0 - Math.pow(centerDist, 2);
        const activeLevel = pulse ? 0.4 : scaledLevel;
        const height = 3 + ((maxBar - 3) * activeLevel * rand * envelope);

        return (
          <div 
            key={i}
            className={`w-1 rounded-full transition-all duration-75 ease-out ${colorClass}`}
